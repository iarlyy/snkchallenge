...
### Overview
The snkchallenge demonstrates how to implement the minimum applications in order to have a reliable distributed network of IoT devices and how they should communicate to each other. 

All components were shipped into docker containers, which brings flexibility to scale and automate the whole solution. 
...
Responsible for receiving all data pushed by the sensors and responsible for providing a channel that the indexer can use to pull the messages sent by the sensors. We are using in this demonstration mosquitto as MQTT broker for your solution.
...
Provides HTTP channel to internet users view all sensors' data already indexed in the database. In this demonstration the Web instance only returns in a JSON format the average of all messages sent by all sensors in the last 5 seconds. The web interface was developed using cherrypy, a minimalist python web framework.
...
In our demonstration we are going to install only the salt-minion and make use of a tool called salt-call that enables we to execute/test saltstates locally without the need of having a salt-master installed.

Our sls(salt state file) contains the full definition of our virtual environment and its dependencies, after having the salt-minion installed and the repository downloaded, we will run only one command and everything will get setup without additional intervention.
